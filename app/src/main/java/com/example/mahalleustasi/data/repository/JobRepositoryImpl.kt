...
class JobRepositoryImpl @Inject constructor(
    ...
    override suspend fun createJob(job: Job): Resource<Unit> {
        return try {
            ...
            if (user == null) {
                return Resource.Error("Oturum açmanız gerekiyor.")
            }

            // Reference için yeni ID oluştur
            val documentRef = firestore.collection("jobs").document()
            
            val jobWithId = job.copy(
                id = documentRef.id,
                postedByUserId = user.uid,
                // Kullanıcı ismini auth profilinden ekle, yoksa email ilk kısım veya "Kullanıcı"
    ...
    override suspend fun getJobById(jobId: String): Resource<Job> {
        return try {
            ...
            if (job != null) {
                ...
                Resource.Error("İlan bulunamadı.")
            }
        } catch (e: Exception) {
